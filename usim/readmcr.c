...
int read_i_mem(int fd, int start, int size)
{
	int i, loc;
	...
	for (i = 0; i < size; i++)
	{
		...
		w3 = read16(fd);
		w4 = read16(fd);
		ll = ((unsigned long long)w1 << 48) | ((unsigned long long)w2 << 32) | ((unsigned long long)w3 << 16) | ((unsigned long long)w4 << 0);
		if (0)
			printf("%03o %016Lo\n", loc, ll);
...
int read_d_mem(int fd, int start, int size)
{
	int i;
	unsigned int v1, v2;
...
int read_a_mem(int fd, int start, int size)
{
	int i;
	unsigned int v;
	...
	{
		v = read32(fd);
		if ((i >= 0347 && i <= 0400) | (i >= 0600 && i <= 0610) | (i < 010))
			printf("%o <- %o\n", i, v);
	}
	return 0;
}

int read_main_mem(int fd, int start, int size)
{
	unsigned int v1;
	off_t o;
	/*unsigned int b[256]; */
