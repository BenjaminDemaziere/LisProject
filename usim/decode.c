...
static unsigned long str4(char *s)
{
	return (s[3]<<24) | (s[2]<<16) | (s[1]<<8) | s[0];
}

static int find_disk_partition_table(int fd)
{
	int ret;
	int p, count, nw, i;
	off_t offset;
...
static unsigned int read_virt(int fd, int addr, unsigned int *pv)
{
	int b;
	off_t offset, ret;
...
static char* read_string(unsigned int loc)
{
	unsigned int v;
	int t, i, j;
	static char s[256];
...
static char* show_string(unsigned int loc)
{
	char *s;
