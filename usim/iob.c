...
unsigned int get_us_clock_low(void)
{
	cv = get_us_clock();
	return cv & 0xffff;
}

unsigned int get_us_clock_high(void)
{
	return cv >> 16;
}
...
void iob_unibus_read(int offset, int *pv)
{
	/* default, for now */
	*pv = 0;
	switch (offset)
...
void iob_unibus_write(int offset, int v)
{
	switch (offset)
	{
		case 0100:
...
int tv_xbus_read(int offset, unsigned int *pv)
{
	if (0)
		printf("tv register read, offset %o -> %o\n", offset, tv_csr);
	
	*pv = tv_csr;
	return 0;
}

int tv_xbus_write(int offset, unsigned int v)
{
	if (0)
		printf("tv register write, offset %o, v %o\n", offset, v);
...
void iob_poll(unsigned long cycles)
{
