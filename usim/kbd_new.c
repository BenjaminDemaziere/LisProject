...
void iob_sdl_key_event(int code, int keydown)
{
...
void iob_sdl_allup_event(int mods)
{
...
static void init_sdl_key_names()
{
	int i;
...
int find_sdl_key_name(char *name)
{
	...
	for (i = SDLK_UNKNOWN; i < SDLK_LAST; i++)
	{
		if ((sdl_key_names[i].sdl_key_name != NULL) && (strcasecmp(sdl_key_names[i].sdl_key_name,name) == 0))
...
int find_lm_key_name(char *name, char is_shift)
{
	...
	{
		if ((strcasecmp(lm_key_names[i].lmkey_name,name) == 0) 	&& (lm_key_names[i].lmkey_is_shift == is_shift))
...
char* find_lm_key_name_name(int num, char is_shift)
{
	...
	for (i = 0; lm_key_names[i].lmkey_name != NULL; i++)
	{
		if ((lm_key_names[i].lmkey_num == num) && (lm_key_names[i].lmkey_is_shift == is_shift))
			return lm_key_names[i].lmkey_name;
	}
	return NULL;
}

static void read_kbd_shiftpress_config(FILE *cf, char *buf, int buflen)
{
	char *eq;
	int sk, lk, i, max = 0;
...
static void read_kbd_keypress_config(FILE *cf, char *buf, int buflen)
{
	char *eq;
	int sk, lk, i, max = 0;
...
static void read_kbd_keyshift_config(FILE *cf, char *buf, int buflen)
{
	unsigned char sk, lk;
	int i, max = 0;
...
static void print_kbd_config()
{
	int i;

	printf(";;Host key => LispM shift key map\n[ShiftKeys]\n");
	for (i = 0; i < SHIFTPRESS_MAP_LEN && (shiftpress_map[i][0] != -1); i++)
		printf("%s=%s\n", SDL_GetKeyName(shiftpress_map[i][0]), find_lm_key_name_name(shiftpress_map[i][1],1));
	
	printf("\n;;Host key => LispM key map\n[Keys]\n");
	for (i = 0; i < KEYPRESS_MAP_LEN && (keypress_map[i][0] != -1); i++)
		printf("%s=%s\n", SDL_GetKeyName(keypress_map[i][0]), find_lm_key_name_name(keypress_map[i][1],0));
...
static int kbd_shiftpress_to_lmcode(SDLKey press)
{
	int i;
...
static int kbd_keypress_to_lmkey(SDLKey press)
{
	int ms, i;
	for (i = 0; i < KEYPRESS_MAP_LEN && (keypress_map[i][0] != -1); i++)
	...
	if ((press >= KBD_SHIFT_MAP_BASE) && (press < 0176))
	{ /* normal keys */
		ms = SDL_GetModState();
#if KBD_DEBUG
			...
			if (state[i] == 1)
			{
				switch (kbd_shiftpress_to_lmcode(i))
				{
					...
					case KB_SH_RIGHT_HYPER:
						...
						break;
					case KB_SH_ALTLOCK:
						mods |= KB_ALLUP_ALTLOCK;
						break;
					case KB_SH_MODELOCK:
						mods |= KB_ALLUP_MODELOCK;
						break;
					default:
...
#endif
			/* If modifiers correct, just post the event, else queue the event and post the appropriate shifts. */
			if (((wantshift == KBNEW_IX_UNSHIFT) && ((kbd_shifts & (1<<KBNEW_IX_SHIFT)) == 0)) || (kbd_shifts & (1<<wantshift)))
				iob_sdl_key_event(lmcode, keydown);
			else
			{
				...
				{
					shkey = (wantshift == KBNEW_IX_SHIFT ? KB_SH_LEFT_SHIFT : (wantshift == KBNEW_IX_TOP ? \
