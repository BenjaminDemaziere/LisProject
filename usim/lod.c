...
unsigned int read_virt(int fd, int addr)
{
	int b;
	off_t offset, ret;
	...
	if (b != bnum)
	{
		bnum = b;
		if (0)
			printf("fd %d, block %d(10) offset %ld\n", fd, b, offset);
...
unsigned int vr(int addr)
{
	return read_virt(lodfd, addr);
}

unsigned int swap_vr(int addr)
{
...
void set_lod1(void)
{
	...
	bnum = -1;
	/* nice hack, eh?  swap starts @ block 0524 - see diskmaker.c */
	// C/C de commentaire de la procédure précédente ?
	partoff = 021210;
	for (i = 0; i < part_count; i++)
	{
...
unsigned int _show(int fd, int a, int cr)
{
	unsigned int v;
...
unsigned int _get(int fd, int a)
{
	unsigned int v;
...
unsigned int get(int a)
{
	return _get(lodfd, a);
}

unsigned int show(int a, int cr)
{
	return _show(lodfd, a, cr);
}

unsigned int showlabel(char *l, int a, int cr)
{
	printf("%s: ", l);
	return _show(lodfd, a, cr);
}

unsigned int showswap(char *l, int a, int cr)
{
	printf("%s: ", l);
	return _show(swapfd, a, cr);
}

unsigned int swap_show(int a, int cr)
{
	return _show(swapfd, a, cr);
}

void showstr(int a, int cr)
{
	int t, i, j;
	unsigned int n;
	char s[256];
...
void show_fef_func_name(unsigned int fefptr)
{
	unsigned int n, v;
	int tag;
...
int find_and_dump_fef(unsigned int pc)
{
