...
int remaining_access(baccess *pb)
{
	int blknum, boff, left;
...
int remaining_buffer(baccess *pb)
{
...
int read_block(int fd, int block_no, char *buf)
{
	off_t offset, ret;
	int size;
...
int read_record(baccess *pb, int record_no)
{
	int i, block_no;
	char *pbuf;
...
int show_file(int fd, struct directory_entry_s *de, int record_no)
{
	baccess b;
...
int show_de(int fd, int record_no)
{
	baccess b;
...
int lmfs_open(char *img_filename, int offset)
{
	int fd, ret;
	u_char buffer[256*4];
  ...
  //  dumpmem((char *)&pl->uid_generator, 64);
	...
	for (i = 0; i < 8; i++)
	{
		...
		if (memcmp(fh->parts_array[i].name, "dire", 4) == 0)
		{
			...
			de = (struct directory_entry_s *)ensure_access(&b, woffset*4, sizeof(struct directory_entry_s));
			printf("file_name '%s', file_type '%s', bytesize %d, author '%s'\n", de->file_name, de->file_type, \
				de->bytesize, de->author);
			printf("number_of_records %d\n", de->number_of_records);
			printf("record_0_address %d\n", de->record_0_address);
...
int lmfs_show_dir(char *path)
{
	return 0;
}

int lmfs_read_file(char *path)
{
	return 0;
}

int lmfs_write_file(char *path)
{
	return 0;
}
