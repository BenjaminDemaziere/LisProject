...
void advance_lc(int *ppc)
{
	...
	if (lc & (1 << 31))
	{
		...
		{
			// condition avec aucune action => lecture du retour de la fonction read sinon warning à la compilation
		}
		new_md_delay = 2;
...
//	 trace_mcr_labels_flag = 1;
		...
		case 022: /* VMA register, start main memory write */
			...
			if (write_mem(vma, md))
			{
				// condition avec aucune action => lecture du retour de la fonction write sinon warning à la compilation
			}
			break;
...
void dump_l1_map()
{
	...
	for (i = 0; i < 2048; i += 4)
	{
		...
		skipped = 0;
		while (l1_map[i+0] == l1_map[i+0+4] && l1_map[i+1] == l1_map[i+1+4] && \
		       l1_map[i+2] == l1_map[i+2+4] && l1_map[i+3] == l1_map[i+3+4] && i < 2048)
		{
			if (skipped++ == 0)
				printf("...\n");
...
void dump_l2_map()
{
	...
	for (i = 0; i < 1024; i += 4) 
	{
		...
		skipped = 0;
		while (l2_map[i+0] == l2_map[i+0+4] && l2_map[i+1] == l2_map[i+1+4] && \
		       l2_map[i+2] == l2_map[i+2+4] && l2_map[i+3] == l2_map[i+3+4] && i < 1024)
		{
			if (skipped++ == 0)
				printf("...\n");
