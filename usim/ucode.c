...
void set_interrupt_status_reg(int new)
{
...
void deassert_unibus_interrupt(int vector)
{
...
void invalidate_vtop_cache(void)
{
 ...
 */
unsigned int map_vtop(unsigned int virt, int *pl1_map, int *poffset)
{
...
/* this should be move below - I'm not sure it has to happen anymore */
	...
	{
		if (0)
			traceio("forcing xbus mapping for disk\n");
 ...
 */
int read_phy_mem(int paddr, unsigned int *pv)
{
	int pn = paddr >> 8;
...
int write_phy_mem(int paddr, unsigned int v)
{
	int pn = paddr >> 8;
	int offset = paddr & 0377;
	struct page_s *page;
...
int write_mem(int vaddr, unsigned int v)
{
	...
	if (pn == 037766)
	{
		...
		if (offset <= 036)
		{
			...
			// switch à un seul case ?? => à reprendre
			switch (offset)
			{
				case 012:
					if ((v & 044) == 044)
...
void write_ucode(int addr, ucw_t w)
{
	tracef("u-code write; %Lo @ %o\n", w, addr);
	ucode[addr] = w;
}

void note_location(char *s, unsigned int v)
{
	printf("%s; u_pc %o, v %o\n", s, u_pc, v);
...
void write_a_mem(int loc, unsigned int v)
{
	//tracef("a_memory[%o] <- %o\n", loc, v);
	a_memory[loc] = v;
}
...
void write_m_mem(int loc, unsigned int v)
{
	m_memory[loc] = v;
	a_memory[loc] = v;
...
#if 0
unsigned int rotate_left(unsigned int v, int rot)
{
	int i, c;
...
unsigned int rotate_left(unsigned int value, int bitstorotate)
{
	unsigned int tmp;
	int mask;
...
// condition avec aucune action ??
	...
	*/
	lc0b = /* byte-mode */ (lc_byte_mode_flag ? 1 : 0) & /* lc0 */ ((lc & 1) ? 1 : 0);
	lc1 = (lc & 2) ? 1 : 0;
 ...
 */
void write_dest(ucw_t u, int dest, unsigned int out_bus)
{
	if (dest & 04000)
	{
...
//	 trace_mcr_labels_flag = 1;
	...
	case 2: /* interrrupt control <29-26> */
		...
		/* preserve flags */
		lc = (lc & ~(017 << 26)) | (interrupt_control & (017 << 26));
		break;
...
void record_pc_history(unsigned int pc, unsigned int vma, unsigned int md)
{
	int index;
...
void show_pc_history(void)
{
	...
	printf("pc history:\n");
	if (0)
		printf("pc_history_ptr %d, pc_history_max %d, pc_history_stores %d\n", pc_history_ptr, pc_history_max, pc_history_stores);
	...
	for (i = 0; i < MAX_PC_HISTORY; i++)
	{
		...
		printf("\tvma %011o md %011o pf%d pdl %o %011o", pc_history[pc_history_ptr].rvma, pc_history[pc_history_ptr].rmd, \
			pc_history[pc_history_ptr].rpf, pc_history[pc_history_ptr].rpdl_ptr, pc_history[pc_history_ptr].rpdl);
#if 1
		printf(" Q %08x M %08x %08x %08x %08x\n", pc_history[pc_history_ptr].rq, pc_history[pc_history_ptr].m[0], \
			pc_history[pc_history_ptr].m[1], pc_history[pc_history_ptr].m[2], pc_history[pc_history_ptr].m[3]);
#endif
		printf("\n");
#if 1
		{
...
void record_pc_histogram(unsigned int pc)
{
...
void dump_l1_map()
{
	...
	for (i = 0; i < 2048; i += 4)
	{
		...
		skipped = 0;
		while (l1_map[i+0] == l1_map[i+0+4] && l1_map[i+1] == l1_map[i+1+4] && l1_map[i+2] == l1_map[i+2+4] && \
			l1_map[i+3] == l1_map[i+3+4] && i < 2048)
		{
			if (skipped++ == 0)
				printf("...\n");
			i += 4;
...
void dump_l2_map()
{
	...
	for (i = 0; i < 1024; i += 4)
	{
		...
		printf("l2[%02o] %011o %011o %011o %011o\n", i, l2_map[i], l2_map[i+1], l2_map[i+2], l2_map[i+3]);
		skipped = 0;
		while (l2_map[i+0] == l2_map[i+0+4] && l2_map[i+1] == l2_map[i+1+4] && l2_map[i+2] == l2_map[i+2+4] && \
			l2_map[i+3] == l2_map[i+3+4] && i < 1024)
		{
			if (skipped++ == 0)
				printf("...\n");
...
void dump_pdl_memory(void)
{
	...
	for (i = 0; i < 1024; i += 4)
	{
		...
		printf("PDL[%04o] %011o %011o %011o %011o\n", i, pdl_memory[i], pdl_memory[i+1], pdl_memory[i+2], pdl_memory[i+3]);
		skipped = 0;
		while (pdl_memory[i+0] == pdl_memory[i+0+4] && pdl_memory[i+1] == pdl_memory[i+1+4] && \
			pdl_memory[i+2] == pdl_memory[i+2+4] && pdl_memory[i+3] == pdl_memory[i+3+4] && i < 1024)
		{
			if (skipped++ == 0)
				printf("...\n");
...
void dump_state(void)
{
...
#endif
	for (i = 0; i < 32; i += 4)
		printf(" spc[%02o] %c%011o %c%011o %c%011o %c%011o\n", i, (i+0 == spc_stack_ptr) ? '*' : ' ', spc_stack[i+0], \
		    (i+1 == spc_stack_ptr) ? '*' : ' ', spc_stack[i+1], (i+2 == spc_stack_ptr) ? '*' : ' ', spc_stack[i+2], \
	...
	for (i = 0; i < /*1024*/01000; i += 4)
		printf("A[%04o] %011o %011o %011o %011o\n", i, a_memory[i], a_memory[i+1], a_memory[i+2], a_memory[i+3]);
...
int breakpoint_set_prom(char *arg)
{
	breakpoint_name_prom = arg;
	return 0;
}

int breakpoint_set_mcr(char *arg)
{
	breakpoint_name_mcr = arg;
	return 0;
}

int breakpoint_set_count(int count)
{
...
int tracelabel_set_mcr(char *arg)
{
	tracelabel_name_mcr = arg;
	return 0;
}

int set_breakpoints(int *ptrace_pt_prom, int *ptrace_pt, int *ptrace_pt_count, int *ptrace_label_pt)
{
	max_cycles = 0;
#if 0
	trace_disk_flag = 1;
...
int set_late_breakpoint(int *ptrace_pt, int *ptrace_pt_count)
{
	if (breakpoint_name_mcr)
	{
		if (sym_find(1, breakpoint_name_mcr, ptrace_pt))
...
//			trace = 1;
		...
		if (stop_after_prom_flag && prom_enabled_flag == 0)
			run_ucode_flag = 0;

		if (trace_prom_flag && prom_enabled_flag == 1)
			trace = 1;

		if (trace_mcr_flag && prom_enabled_flag == 0)
			trace = 1;
		...
		switch (op_code = (u >> 43) & 03)
		{
			case 0: /* alu */
				...
				if (trace)
				{
					...
					printf("alu_op %o, ir8 %o, ir7 %o, c %o, dest %o, out_bus %d\n", alu_op, ir8, ir7, carry_in, dest, out_bus);
				}
...
//new - better?
//							alu_out = (m_src_value << 1) | (carry_in ? 1 : 0);
...
#endif
					break;
				case 3: out_bus = (alu_out << 1) | ((old_q & 0x80000000) ? 1 : 0);
					break;
			}
